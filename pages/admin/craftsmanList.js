...
Page({
  ...
  async loadList(refresh = false) {
    ...
    try {
      const res = await API.call('admin', 'getCraftsmenList', {
        page: this.data.page,
  ...
  async onSaveEdit() {
    ...
    try {
      const res = await API.call('admin', 'updateCraftsman', {
        id: editingId,
        ...editForm
      })
    ...
    wx.showModal({
      ...
      success: async (res) => {
        if (res.confirm) {
          ...
          try {
            // 实际删除应该是更新状态而不是物理删除
            const result = await API.call('admin', 'updateCraftsman', {
              id,
              status: 'disabled'
            })
