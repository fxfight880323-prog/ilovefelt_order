...
CREATE TABLE users (
    ...
    role VARCHAR(20) DEFAULT 'guest' COMMENT '主角色 guest/craftsman/dispatcher/admin',
    roles JSON COMMENT '所有角色数组 ["guest"]',
    current_role VARCHAR(20) DEFAULT 'guest' COMMENT '当前角色',
    phone VARCHAR(20) COMMENT '手机号',
    status VARCHAR(20) DEFAULT 'active' COMMENT '状态 active/pending',
...
CREATE TABLE orders (
    ...
    -- 接单时间
    accept_date TIMESTAMP NULL DEFAULT NULL COMMENT '接单时间',
...
('欢迎使用手工艺派单系统', '本系统用于手工艺品派单管理，支持手艺人接单和派单人发布订单。', 'normal', 1),
('手艺人注册须知', '注册为手艺人需要填写真实信息并通过审核，审核通过后方可接单。', 'important', 1);
